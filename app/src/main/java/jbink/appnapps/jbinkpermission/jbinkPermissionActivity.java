...
import jbink.appnapps.jbinkpermission.busevent.jbinkBusProvider;
import jbink.appnapps.jbinkpermission.busevent.jbinkPermissionEvent;

/**
 * Created by user on 2016-10-14.
 */
public class jbinkPermissionActivity extends AppCompatActivity{
    ...
    private void permissionGranted(){
        jbinkBusProvider.getInstance().post(new jbinkPermissionEvent(true, null));
        finish();
        overridePendingTransition(0,0);
    }

    private void permissionDenied(ArrayList<String> deniedpermissions){
        jbinkBusProvider.getInstance().post(new jbinkPermissionEvent(false, null));
        finish();
        overridePendingTransition(0,0);
    }


    private void checkPermissions(boolean fromOnActicityResult){
    ...
    public void showPermissionDenyDialog(final ArrayList<String> deniedPermissions) {
        ...
        if (hasSettingButton) {
            if(TextUtils.isEmpty(settingButtonText)){
                settingButtonText = "tedpermission_setting";
